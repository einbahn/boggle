...
public class TrieSET implements Iterable<String> {
    ...
    private Node add(Node x, String key, int d) {
        ...
        if (d == key.length()) {
            ...
            char c = key.charAt(d);
            x.next[alpha.toIndex(c)] = add(x.next[alpha.toIndex(c)], key, d+1);
        }
        return x;
    }
    ...
    private void collect(Node x, StringBuilder prefix, Queue<String> results) {
        ...
        for (char c = 0; c < R; c++) {
            char achar = alpha.toChar(c);
            prefix.append(achar);
            collect(x.next[alpha.toIndex(achar)], prefix, results);
    ...
    private void collect(Node x, StringBuilder prefix, String pattern, Queue<String> results) {
        ...
        if (c == '.') {
            for (char ch = 0; ch < R; ch++) {
                prefix.append(alpha.toChar(ch));
                collect(x.next[alpha.toIndex(alpha.toChar(ch))], prefix, pattern, results);
                prefix.deleteCharAt(prefix.length() - 1);
            }
        }
        else {
            prefix.append(c);
            collect(x.next[alpha.toIndex(c)], prefix, pattern, results);
            prefix.deleteCharAt(prefix.length() - 1);
        }
    }
    ...
    private int longestPrefixOf(Node x, String query, int d, int length) {
        ...
        return longestPrefixOf(x.next[alpha.toIndex(c)], query, d+1, length);
    }
    ...
    private Node delete(Node x, String key, int d) {
        ...
        if (d == key.length()) {
            ...
            char c = key.charAt(d);
            x.next[alpha.toIndex(c)] = delete(x.next[alpha.toIndex(c)], key, d+1);
        }

        // remove subtrie rooted at x if it is completely empty
        if (x.isString) return x;
        for (char c = 0; c < R; c++)
            if (x.next[alpha.toIndex(c)] != null)
                return x;
        return null;
    }
    ...
    public static void main(String[] args) {
        TrieSET set = new TrieSET(Alphabet.LOWERCASE);
